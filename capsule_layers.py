...
def squash(vectors, axis=-1):
        ...
        """
        # Change K.sum to tf.reduce_sum
        s_squared_norm = tf.reduce_sum(tf.square(vectors), axis, keepdims=True)
        scale = s_squared_norm / (1 + s_squared_norm) / tf.sqrt(s_squared_norm + K.epsilon()) # change here as well
...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding):
    ...
    # Define output_shape function for the Lambda layer
    def output_shape(input_shape):
        return input_shape

    # Pass the output_shape function to the Lambda layer
    return layers.Lambda(squash, output_shape=output_shape, name='primarycap_squash')(outputs)
